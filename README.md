...
Using the full ADC and PMW range effectively may require some balancing.  A convenient degree of freedom is the introducion of calibration_factor in the ESP32 code (line 32) which should be adjusted for best accuracy.  Note that the transformer has a very small load so the magnetising current is significant compared to load resulting an a phase error.  This is constant and can be accounted for in calibration_factor.  Note that the PMW filter has a phase shift of 22 deg at 50Hz which represents a PF of 0.92.   Compensation for this effect is lumped within calibration_factor.
